...
namespace danielebuso\sanitizer\Laravel;
...
class Factory
{
    ...
    public function extend($name, $customFilter)
    {
        ...
        if (!($customFilter instanceof Closure) && !in_array(Filter::class, class_implements($customFilter))) {
            throw new InvalidArgumentException('Custom filter must be a Closure or a class implementing the danielebuso\sanitizer\Contracts\Filter interface.');
        }
